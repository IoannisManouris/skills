...
## Required final performance report

After the entire optimization process is complete, include a final performance change report table. The table must summarize every meaningful optimization that was made or recommended and show before/after values whenever they are measured, directly observed, or safely computable from the code.
...
Use concrete units where possible, such as ms/frame, CPU %, MB, KB/s, remote calls/sec, payload bytes, Instance count, connection count, allocation count, loop frequency, FPS, load time, or script profiler time.

Calculate `Improvement %` only from real before/after numbers:
- For lower-is-better metrics, such as CPU time, memory, remotes/sec, payload size, allocations, Instance count, load time, or frame time: `(before - after) / before * 100`.
- For higher-is-better metrics, such as FPS, throughput, or successful frame budget headroom: `(after - before) / before * 100`.
- If `before` is zero, missing, qualitative, or not comparable, write `N/A` instead of a percentage.

A positive `Improvement %` should mean the result improved. A negative percentage should mean the metric got worse or there is a tradeoff.

Never fabricate performance percentages. If profiling was not run, still include the table, but mark measured values as `Not measured`, percentages as `N/A`, and explain the expected direction in `Notes`.

Example table format:

| Area / metric | Category | Before | After | Change | Improvement % | Evidence / source | Notes |
|---|---:|---:|---:|---:|---:|---|---|
| Remote update frequency | Networking | 60 calls/sec/player | 10 calls/sec/player | -50 calls/sec/player | 83.3% | Code path calculation | Batched updates; verify in Developer Console network stats. |
| Heartbeat scan loop | Algorithmic / frame path | Full Workspace scan every frame | Cached tagged set updated by signals | N/A | N/A | Code review | Expected to reduce frame spikes; profile with Script Profiler. |
...
7. End with the required final performance report table showing before values, after values, change, and improvement percentage where measurable.

Final rule: performance optimization is successful only when the experience remains correct, secure, maintainable, and measurably faster. Do not claim a percentage improvement unless it comes from measured or safely computable before/after values.
